                                ...
                                <div class="row">
                                    <div class="col-lg-6">
                                                ...
                                                <div class="profile-name">
                                                    ...
                                                    @if(Auth::check())
                                                        @if($user->id == Auth::User()->id)
                                                            <a class="btn pjax" href="{{ route('profile.edit') }}">{{ __('Edit Profile') }}</a>
                                                        @else
                                                            @if(Auth::User()->isFollowing($user->id))
                                                                <a class="btn pjax" href="{{ route('unfollow',$user->id) }}">{{ __('Unfollow') }}</a>
                                                                <a class="btn pjax"  onclick="popup('{{ $user->id }}')" loop muted="muted" onmouseover="mouseover('{{ $user->id }}')" onmouseout="mouseout('{{ $user->id }}')">{{ __('Message') }}</a>
                                                            @else
                                                                <a class="btn pjax" href="{{ route('follow',$user->id) }}">{{ __('Follow') }}</a>
                                                                <a class="btn pjax"  onclick="popup('{{ $user->id }}')" loop muted="muted" onmouseover="mouseover('{{ $user->id }}')" onmouseout="mouseout('{{ $user->id }}')">{{ __('Message') }}</a>
                                                            @endif
                                                        @endif
                                                    @else
                                        ...
                                        <div class="user-follow-info">
                                            ...
                                            <span><strong>{{ App\Helpers\UserSystemInfo::conveter($user->views) }}</strong>{{ __('Views') }}</span>
                                        </div>
                                        @php 
...
<!-- main area end -->
<input type="hidden" id="popup_url" value="{{ route('popup.message') }}">
